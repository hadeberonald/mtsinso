...
function buildVehicleWhatsAppUrl(vehicle: Vehicle) {
  ...
  const message =
`Hi IC Cars! 👋 I'm interested in the following vehicle:
...
function Lightbox({
  ...
  return (
    /* Full-screen backdrop — click backdrop to close */
    <div
    ...
    >
      ...
      >
        ...
        {/* Big tap target for close — critical on mobile */}
        <button
          onClick={onClose}
        ...
        {imgs.length > 1 && (
          <button
            ...
            className="absolute left-2 sm:left-5 top-1/2 -translate-y-1/2 z-10
                       ...
                       bg-black/60 hover:bg-gold active:bg-gold
      ...
      {imgs.length > 1 && (
        <div
          className="flex-shrink-0 bg-black/80 py-3 px-4"
          onClick={e => e.stopPropagation()}
        >
          <div className="flex gap-2 justify-center overflow-x-auto pb-1">
            {imgs.map((img, i) => (
              <button
                key={i}
                onClick={() => setIdx(i)}
                className={`relative h-14 w-20 flex-shrink-0 border-2 transition-all overflow-hidden touch-manipulation
                  ${i === idx ? 'border-gold' : 'border-white/20 hover:border-gold/60'}`}
              >
...
export default function VehicleDetailPage() {
  ...
  if (!vehicle) return (
    <div className="pt-20 min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <Link href="/vehicles" className="mt-4 inline-block btn-hollow btn-hollow-gold">Back to Vehicles</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="pt-20 min-h-screen bg-white">
      ...
      {/* ── Lightbox (portal-style, rendered at top level) ── */}
      ...
      <div className="bg-black py-4">
        <div className="max-w-7xl mx-auto px-6">
          <Link href="/vehicles" className="inline-flex items-center text-white hover:text-gold transition-colors text-sm">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            {/* Main image */}
            <div
              className="relative h-96 md:h-[500px] mb-3 card-angled overflow-hidden border-2 border-dark-light cursor-pointer group"
            ...
            >
              {imgs.length > 0 ? (
                <>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/30">
                    <div className="bg-white text-black text-sm px-4 py-2 font-bold tracking-wide">
                      Click to Enlarge
                    </div>
                  </div>
                  {/* In-gallery prev/next — stop propagation so they don't open lightbox */}
                  {imgs.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-4 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center
                                   bg-black/50 border border-white/50 text-white
              ...
              {/* Condition badge */}
              <div className="absolute top-4 left-4 bg-gold text-black px-3 py-1 font-bold uppercase text-xs tracking-wide">
                {vehicle.condition}
              </div>
              {/* Counter */}
              {imgs.length > 1 && (
                <div className="absolute bottom-4 right-4 bg-black/60 text-white text-xs px-2 py-1 font-medium">
              ...
              <div className="grid grid-cols-6 gap-1.5">
                {imgs.map((img, i) => (
                  <button
                    ...
                    className={`relative h-16 card-angled overflow-hidden border-2 transition-all hover:scale-105 touch-manipulation
                      ${i === currentImageIndex ? 'border-gold' : 'border-dark-light hover:border-gold/60'}`}
                  >
                    <Image src={img} alt="" fill className="object-cover" sizes="80px" />
                  </button>
                ))}
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <h1 className="text-3xl md:text-4xl font-display text-dark mb-4 leading-tight">
                {vehicle.year} {vehicle.make} {vehicle.model}
              </h1>
              <div className="flex items-baseline gap-4 flex-wrap">
                <span className="text-4xl font-bold text-gold">{formatPrice(vehicle.price)}</span>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[
                ...
                <div key={label} className="card-angled bg-gray-50 p-4 border-2 border-dark-light">
            ...
            {/* Specs table */}
            <div className="card-angled bg-gray-50 p-5 border-2 border-dark-light mb-6">
              <h3 className="text-base font-bold text-dark mb-3 pb-2 border-b border-gray-200">
                Specifications
              </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href={financeUrl} className="flex-1">
                <button className="w-full btn-filled btn-filled-gold font-bold tracking-wide">
                  Get Pre-Approved
                </button>
              </Link>
              ...
              {/* WhatsApp inquiry */}
              <a
                href={buildVehicleWhatsAppUrl(vehicle)}
                target="_blank"
              ...
              >
                <button
                  className="w-full flex items-center justify-center gap-2
                             bg-[#25D366] hover:bg-[#1da851] active:bg-[#199c47]
                             text-white font-bold tracking-wide px-6 py-3
            ...
            <p className="text-xs text-gray-400 mt-2 text-center">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="underline hover:text-[#25D366] transition-colors"
              >
        ...
        {/* ── Description ── */}
        <div className="mt-16">
          <h2 className="text-3xl font-display text-dark mb-5">Vehicle Description</h2>
          <div className="card-angled bg-gray-50 border-2 border-dark-light p-8">
        ...
        {/* ── Features & Equipment ── */}
        <div className="mt-16">
          <h2 className="text-3xl font-display text-dark mb-5">Features & Equipment</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {Object.entries(FEATURE_CATEGORIES).map(([catKey, cat]) => {
              ...
              return (
                <div key={catKey} className="card-angled bg-white border-2 border-dark-light p-6">
                  <h3 className="text-lg font-display text-dark mb-4 pb-2 border-b-2 border-gold">
                    {cat.title}
                  </h3>
                  <div className="space-y-2">
                    ...
                    {enabled.map(f => (
                      <div key={f.key} className="flex items-center gap-2 text-sm text-dark">
                        <Check className="w-4 h-4 text-green-500 flex-shrink-0" />
                        {f.label}
                      </div>
                    ))}
                    {catKey === 'history' && (
                      <div className="flex items-center gap-2 text-sm text-dark mt-2 pt-2 border-t border-gray-100">
        ...
        {(vehicle.service_history || vehicle.warranty || vehicle.vin || vehicle.license_plate) && (
          <div className="mt-16">
            <h2 className="text-3xl font-display text-dark mb-5">Vehicle History & Documentation</h2>
            <div className="card-angled bg-gray-50 border-2 border-dark-light p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {vehicle.service_history && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-dark mb-0.5">Full Service History</h4>
                      <p className="text-sm text-gray-500">Complete service records available</p>
                    </div>
                  </div>
                )}
                {vehicle.warranty && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-dark mb-0.5">Warranty Coverage</h4>
                ...
                {vehicle.vin && (
                  <div>
                    <h4 className="font-bold text-dark mb-0.5">VIN Number</h4>
                    <p className="text-sm text-gray-400 font-mono">{vehicle.vin}</p>
                  </div>
                )}
                {vehicle.license_plate && (
                  <div>
                    <h4 className="font-bold text-dark mb-0.5">License Plate</h4>
                    <p className="text-sm text-gray-400 font-mono">{vehicle.license_plate}</p>
                  </div>
                )}
                {vehicle.registration_date && (
                  <div>
                    <h4 className="font-bold text-dark mb-0.5">First Registration</h4>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            <div key={title} className="card-angled bg-black text-white p-6 border-2 border-gold">
              <h3 className="text-lg font-display text-gold mb-2">{title}</h3>
